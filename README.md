...
Apply migrations through `202609170014_widget_configuration.sql`; older migrations remain unchanged. Current validation: **436 backend tests / 177 frontend tests**, Ruff lint/format, frontend lint/build and Git whitespace checks pass. The build reports a non-blocking existing application chunk-size warning; the standalone launcher does not bundle React. Migration 014 was dry-run reviewed as the only pending migration and applied to linked Free Supabase without reset. Rolled-back synthetic hosted pgTAP checks passed: widget configuration (25 assertions), admin operations (86), customer chat (48). This is selected coverage, not the full database suite. An independent demo at port 4174 was checked for launcher, host-style isolation, iframe loading, close/reopen, Escape and host refresh. Its unused synthetic identifier showed safe unavailable behavior; customer-chat server secret and Gemini are not configured, so live sessions/answers and authenticated browser toggles were not tested. Phase 8B and deployment remain pending. See [widget integration](docs/WIDGET_INTEGRATION.md).
