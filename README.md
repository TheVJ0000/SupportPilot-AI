...
**Phase 10A — zero-cost deployment readiness complete (September 19, 2026).** A reviewed Render Blueprint now defines one Free FastAPI web service and two free static sites for the React application and independent-origin widget demo. Production builds reject server-secret names/markers in the browser bundle; GitHub Actions validates backend, frontend, Chromium E2E and deterministic offline RAG without live providers. Deployment, public URLs, production CSP and post-deployment smoke tests have **not** happened yet. Automatic Render deploys are off until the first successful smoke. See the [deployment guide](docs/DEPLOYMENT.md) and [synthetic demo-data plan](docs/DEMO_DATA.md).
...
**Phase 9C — deterministic evaluation complete; live embeddings/hosted retrieval validated, generation incomplete (September 18, 2026).** The 36-case fictional benchmark separately measures retrieval, decisions, facts, citations, injection and isolation. Offline lexical Hit@8 is 100%, Recall@8 98.39%, with one retained miss; scripted answer checks are **not live model quality**. A separate real Gemini-embedding/hosted pgvector run passed: Hit@1 93.55%, Hit@8 and Recall@8 100%, MRR 0.96774, Beta positive control and rollback verified, no retrieval isolation leak. A fixture-role bug and installed-SDK request-format bug were corrected with regressions. The original full run failed generation HTTP 400; the corrected bounded run stopped on HTTP 503 after existing retries, with no successful answer/stream. **583 backend / 203 frontend / 24 Chromium tests pass**, plus lint/format/build. Prompts, models, strict local validation, thresholds and database schema/grants remain unchanged; no migration 017, new dependency or paid service. See [actual results and failure history](docs/RAG_EVALUATION.md).

Phase 9 as a whole is **not** complete. Gemini is privately configured on the verified free project with billing disabled, but successful live generation remains unverified after provider failure. `SUPABASE_SECRET_KEY` is still missing, blocking the real customer HTTP/SSE/persistence journey; optional triage was not run. All synthetic hosted fixtures were rolled back. No email or paid billing was enabled. Possible 9D has no demonstrated load/resource requirement and was not started; sequential retrieval timings and serial browser tests do not prove capacity. Storage API byte deletion, aggregate AI-budget/bot protection, shared-widget availability and compromised-key/XSS risks remain documented. Phase 10A prepares deployment but does not claim a live deployment; CSP, platform-log review and one bounded post-deployment generation check remain. The bundle-size warning remains non-blocking.
...
website. Phase 10A adds deployment configuration only; no public address exists
until the Render Blueprint is created and manually smoke-tested.
