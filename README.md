...
**Phase 10B — public portfolio deployment complete (September 21, 2026).** The reviewed Blueprint is live on Render Free: [SupportPilot AI](https://supportpilot-web-6w61.onrender.com), [FastAPI health](https://supportpilot-api-ytih.onrender.com/api/health), and the [external widget demo](https://supportpilot-widget-demo.onrender.com). The production smoke covered Supabase Auth/session restoration, a synthetic workspace, five processed/indexed knowledge sources, real Gemini embeddings and grounded streaming generation, trusted citations, safe insufficient evidence, feedback, history reload, human handoff, admin views, resolve/reopen lifecycle, and external-origin widget isolation. Exact-origin CORS and static security headers passed; the public bundle contains only browser-safe configuration. All three Render resources remain free, manual deploys remain enabled, and no Render database, disk, queue, paid plan, custom domain, or keep-alive was added. See the [deployment guide](docs/DEPLOYMENT.md) and [synthetic demo-data plan](docs/DEMO_DATA.md).
...
**Phase 9C — complete after bounded deployed validation (September 21, 2026).** The earlier deterministic and live-retrieval results remain unchanged: the 36-case fictional benchmark has offline lexical Hit@8 100% and Recall@8 98.39%, while the separate real Gemini-embedding/hosted pgvector run achieved Hit@1 93.55%, Hit@8 and Recall@8 100%, MRR 0.96774, with the Beta isolation control and rollback passing. Phase 10B then supplied the missing bounded real-customer path: one supported production question streamed a grounded answer with the correct trusted FAQ citation, and one deliberately absent price-match question returned the controlled insufficient-evidence response without inventing a policy. This is a small synthetic smoke, not a broad live-model quality claim. See [actual results and failure history](docs/RAG_EVALUATION.md).

Phase 9 is **complete at its defined validation baseline**. The production customer journey now proves authenticated setup, real embedding/indexing, pgvector retrieval, structured Gemini generation, SSE delivery, citation reconstruction, persistence, feedback, history reload and safe insufficiency. Phase 9D remains **not justified** because no concrete capacity or concurrent-stream failure was observed. The bounded triage worker created the durable escalation; two transient provider-unavailable audits were followed by a successful third automatic attempt that classified the synthetic policy issue as low priority. No email was sent because optional Resend configuration remains unset. Storage API byte deletion, aggregate AI-budget/bot protection, shared-widget availability, compromised-key/XSS risks, Render cold starts, free-tier worker sleeping and the absence of a verified CSP remain documented limitations rather than invented assurances.
...
This makes local startup repeatable, but it does not turn localhost into the
public deployment. The public portfolio application is available at
`https://supportpilot-web-6w61.onrender.com`; local development remains useful
for implementation and offline regression work.
