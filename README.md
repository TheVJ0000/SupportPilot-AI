...
Apply migrations through `202609170015_public_api_hardening.sql`. Current validation passes **489 backend / 201 frontend tests**, Ruff lint/format, frontend lint/build and Git whitespace checks. Migration 015 was the only pending migration in dry run; its 60-assertion rollback preflight passed before normal hosted application. Hosted transactional pgTAP suites actually rerun: public API/rate limits (60), customer chat (48), widget configuration (25); not the full database suite. The bounded synthetic ASGI API smoke accepted 30 fixture requests, returned 429 with Retry-After, then succeeded in a fresh test window. This used an explicit test gateway, not live database-backed HTTP traffic. Live health/frontend/demo responded 200; the unconfigured live session endpoint returned safe 503, and the external-origin widget shell/unavailable/close/reopen checks passed. An optional two-connection last-slot probe was inconclusive because one CLI initialization stalled and was cancelled; its temporary synthetic counter was removed. No live customer/Gemini journey is claimed. Fixed-window boundary bursts, shared-widget availability, production/load tuning and comprehensive concurrency verification remain Phase 9; deployment framing/CSP remains Phase 10. Neither phase was started. The existing non-blocking application bundle-size warning remains.
