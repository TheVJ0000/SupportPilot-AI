...
function ApiStatus({ status }: { status: AuthenticatedApiStatus | 'checking' }) {
  ...
  return (
    <div aria-live="polite" className="flex items-center gap-2 text-xs text-slate-400">
      <span className={`size-2 rounded-full ${colors[status]} ${status === 'checking' ? 'animate-pulse' : ''}`} />
      {labels[status]}
    </div>
  )
}
      ...
      <header className="border-b border-white/10 bg-slate-950/30 px-5 py-4 backdrop-blur-xl sm:px-8">
        <div className="mx-auto flex max-w-6xl items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-4">
            ...
            <button className="rounded-xl border border-white/10 px-4 py-2 text-sm font-semibold text-slate-300 hover:bg-white/5 hover:text-white focus:outline-none focus-visible:ring-2 focus-visible:ring-cyan-300" onClick={() => void handleSignOut()} type="button">
              Log out
            </button>
          </div>
        </div>
