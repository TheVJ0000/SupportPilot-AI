...
The hosted UI renders all customer, assistant, and citation text as plain text. Citation locations support PDF pages, DOCX blocks, text/Markdown lines, and FAQs without inventing source URLs. The streaming turn endpoint completes the existing grounded generation and atomic persistence path before emitting bounded `answer_delta` events followed by one validated `complete` event as NDJSON. This preserves trusted structured output, citations, and idempotent retries while progressively rendering delivery in the browser; it does not claim lower model-generation latency. The UI restores feedback and handoff state from the backend, uses inline confirmation before handoff, and disables the composer afterward without claiming that a person is connected or notified. Phase 6 will add triage/escalation automation.
